...
LogFileAppender::LogFileAppender(const char* file_name, bool async, int roll_size, int flush_interval, int check_everyn, bool thread_safe)
    ...
    if (m_async) {
        m_async_logging = new AsyncLogging(std::bind(&LogFileAppender::fwrite, this, std::placeholders::_1), m_flush_interval);
...
void LogFileAppender::fflush_unlocked() {
    if (m_file) {
        ::fflush(m_file);
        m_last_flush = ::time(nullptr);
    }
}
